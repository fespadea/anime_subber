...
- Vertical Japanese OCR is read top-to-bottom within a column and right-to-left across adjacent columns. Do not reorder tategaki into left-to-right column order.
- OCR placement belongs in ASS (`\\pos`); SRT position tags are compatibility-only approximations.
- `--ocr-only` must preserve existing cues in either project-generated SRT or ASS output before adding refreshed OCR cues.
